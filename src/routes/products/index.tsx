	...
	const filteredProducts = products.filter((product) => {
		return filterItems.every((item) => {
			...
			return product.options.some((option) => {
				const value = option.attributes[
					item.value as keyof typeof option.attributes
				] as string;
			...
			<main className="space-y-6">
				...
				<Pagination>
					<PaginationContent>
						...
						<PaginationItem>
							<PaginationPrevious
								onClick={() => goToPage(Math.max(search.page - 1, 1))}
							/>
						</PaginationItem>
						...
						<PaginationItem>
							<PaginationNext
								onClick={() => goToPage(Math.min(search.page + 1, totalPages))}
							/>
						</PaginationItem>
					</PaginationContent>
				</Pagination>
