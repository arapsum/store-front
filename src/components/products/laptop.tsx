...
function RenderLaptop({ product }: Props) {
  ...
  const availableColours = unique(options.map((option) => option.attributes.colour));
  const availableMemory = unique(options.map((option) => option.attributes.memory));
  const availableStorage = unique(options.map((option) => option.attributes.storage));
  ...
  const selectAttribute = (attribute: Partial<LaptopAttribute>) => {
    ...
    if (!match) {
      ...
      match = options.find((o) => o.attributes[changedKey] === attribute[changedKey]);
    }
  ...
  return (
    <section className="flex flex-col gap-0">
      <div className="hidden md:block">
        <ProductDetailsBreadcrumbs brand={product.brand} category={product.category} />
      </div>
            ...
            <section className="flex flex-col space-y-6">
              <div className="space-y-2">
                ...
                <h1 className="text-xl leading-6 md:text-2xl text-foreground font-heading">
                  {product.brand} {product.name} - {attributes.screenSize} - {attributes.processor}{" "}
                  - {attributes.memory} RAM - {attributes.storage} Storage - {attributes.colour}
                </h1>

                <div className="flex gap-1">
                  {Array.from({ length: 4 }).map((_, index) => (
                    <StarIcon key={index} className="text-amber-500 size-4 fill-amber-500" />
                  ))}
          ...
          <section className="py-10 px-4 mx-auto rounded-sm bg-[#FAFAFA]">
            <main className="flex flex-col gap-8 py-12 px-6 bg-white rounded-sm">
              ...
              <div className="w-full">
                <h1 className="text-2xl font-medium leading-8 text-black">Details</h1>
              </div>
              {/* Description */}
              <div>
