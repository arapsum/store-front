import type { CategoryKey, Option, Specification, SpecificationEntry } from "#/types/product";
...
function RenderSpecifications<T extends CategoryKey>({ specifications, option }: Props<T>) {
        ...
        {visibleSpecs.map(
          ([key, value]: [string, Record<string, SpecificationEntry<T>>], index: number) => (
            <div key={`${key}-${index}`} className="flex flex-col gap-4">
              <div>
                <h5 className="font-medium leading-6 text-black capitalize text-[20px]">{key}</h5>
              </div>
