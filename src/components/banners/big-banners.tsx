...
function BigBanner({
  ...
  const isDarkBackground =
    buttonVariant === "dark" || titleColour.includes("white") || titleColour.includes("#FFFFFF");
    ...
    <div className={`px-8 pb-14 h-full w-full  ${bgColour}`}>
      <div className="flex flex-col space-y-6 h-full md:justify-between">
        <div className="flex flex-col gap-6">
          {/* Image Section */}
          <div className="flex justify-center items-center w-full h-60 shrink-0">
            <img
              src={imageSrc}
              alt={imageAlt}
              width={imageWidth}
    ...
    <section className="h-auto bg-white w-svw xl:w-360">
      ...
      <div className="relative">
        ...
        <div className="overflow-hidden space-y-12 md:hidden">
          ...
          >
            {banners.map((banner) => (
              <div key={banner.title} className="w-full shrink-0">
                <BigBanner {...banner} />
              </div>
            ))}
          </div>
        ...
        {/* Desktop grid */}
        <div className="hidden gap-0 w-full h-full md:grid md:grid-cols-2 lg:grid-cols-4 shrink-0">
          {banners.map((banner) => (
